...
// Helper: construir resultado final
const devolver = (codigo, motivo, esNum = false) => {
...
// ── 1. Numérico puro → es un botón del menú, conservar idioma existente en BD ─
if (/^\d+$/.test(mensaje)) {
  const cod = mapaIdiomas[normCode] ? normCode : 'es';
  return devolver(cod, `Numérico → Telegram: ${telegramCode}`, true);
}

// ── 2. Scripts no-latinos → certeza absoluta ──────────────────────────────────
if (/[Ѐ-ӿ]/.test(mensaje)) {
...
// ── 3. Puntuación — texto en script latino ────────────────────────────────────
let scores = {};
for (const c of Object.keys(mapaIdiomas)) scores[c] = 0;

// Telegram language_code — peso 3 (tiebreaker, NO dominante sobre el texto)
if (normCode && mapaIdiomas[normCode]) scores[normCode] += 3;

// Diacríticos exclusivos — peso 8 (certeza muy alta)
...
const strongKW = {
  es: ['hola','buenas','buenos días','buenos dias','¿hola','¡hola'],
...
const keywords = {
  ...
  it: ['ho un','ho una','ho bisogno','bisogno','voglio','aiuto','grazie',
       'emergenza','problema','perdita','acqua','idraulico','prezzo',
       'preventivo','rotto','rotta','rubinetto','tubo','non funziona'],
...
if (maxScore === 0) {
  ...
  return devolver(fallback, `Sin señales → Telegram: ${telegramCode}`);
}
